        ...
        >
          {[
            ...
            <div key={unit.label} className="text-center">
              <motion.div
                className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-birthday-pink/20 px-4 py-3 md:px-6 md:py-4 border-2 border-birthday-pink/30"
                animate={{ scale: unit.label === 'Secs' ? [1, 1.05, 1] : 1 }}
                transition={{ duration: 1, repeat: Infinity }}
              >
                <span className="text-3xl md:text-5xl font-display font-black bg-gradient-to-b from-birthday-pink to-birthday-coral bg-clip-text text-transparent">
                  {String(unit.value).padStart(2, '0')}
                </span>
              </motion.div>
              <p className="text-xs md:text-sm font-display font-bold text-birthday-lavender mt-1">
                {unit.label}
              </p>
            </div>
          ))}
